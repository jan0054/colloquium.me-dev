...
public class PeopleDetailsActivity extends BaseActivity {
    ...
    public void processConversation (final ParseUser theguy)
    {
		...
		ParseQuery<ParseObject> conv = ParseQuery.getQuery(_PARAMS.TABLE_CONVERSATION);
		ArrayList<ParseUser> cond_relation = new ArrayList<ParseUser>();
		cond_relation.add(currentUser);
		cond_relation.add(theguy);
		conv.whereContainsAll(ConversationAdapter.PARTICIPANTS, cond_relation);
		conv.whereNotEqualTo(ConversationAdapter.ISGROUP, 1);
		Log.d("conversation", currentUser.getUsername());
		Log.d("conversation", theguy.getUsername());
        ...
        mainQuery.getFirstInBackground(new GetCallback<ParseObject>() {
            ...
            public void done(ParseObject object, final ParseException e) {
                if (e == null) {
					...
					Log.d("conversation get one", conv_objid);
                    goChat(conv_objid);
                }
                else
                {
                    ...
                    if (e.getMessage().equals("no results found for query"))
                    {
                        ...
                        ParseObject new_conv = new ParseObject(_PARAMS.TABLE_CONVERSATION);
                        new_conv.put(ConversationAdapter.LASTMSG, "no messages yet");
                        Date date = new Date();
                        new_conv.put(ConversationAdapter.LASTTIME, date);
						ArrayList<ParseUser> user_array = new ArrayList<ParseUser>();
                        user_array.add(currentUser);
						user_array.add(theguy);
						new_conv.put(ConversationAdapter.PARTICIPANTS, user_array);
						new_conv.put(ConversationAdapter.ISGROUP, 0);
                        new_conv.saveInBackground(new SaveCallback() {
							@Override
							public void done(ParseException ee) {
								if (ee == null) {
									//new conversation created, get the id
									ParseQuery<ParseObject> newconvquery = ParseQuery.getQuery(_PARAMS.TABLE_CONVERSATION);
									ArrayList<ParseUser> cond_relation = new ArrayList<ParseUser>();
									cond_relation.add(currentUser);
									cond_relation.add(theguy);
									newconvquery.whereContainsAll(ConversationAdapter.PARTICIPANTS, cond_relation);
									newconvquery.whereNotEqualTo(ConversationAdapter.ISGROUP, 1);
