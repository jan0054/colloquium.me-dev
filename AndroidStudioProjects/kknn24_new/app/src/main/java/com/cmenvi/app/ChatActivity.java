...
public class ChatActivity extends BaseActivity implements  AdapterView.OnItemClickListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        ParseQuery<ParseObject> query = ParseQuery.getQuery(_PARAMS.TABLE_CONVERSATION);
        query.getInBackground(conversationid, new GetCallback<ParseObject>() {
            public void done(ParseObject object, ParseException e) {
                if (e == null) {
                    ...
                    List<ParseUser> participant = pfconversation.getList(ConversationAdapter.PARTICIPANTS);
                    pfconversation.saveInBackground();
                } else {
                    // something went wrong
                    Log.i("create conversation", "fail"+e);
                }
            }
        });
    ...
    public void sendChat(View view) {
        ...
        ParseObject chatmsg = new ParseObject(_PARAMS.TABLE_CHAT);
        chatmsg.put(ChatAdapter.AUTHOR, currentUser);
        chatmsg.put(ChatAdapter.CONTENT, message);
        if (pfconversation == null) {
            pfconversation = new ParseObject(_PARAMS.TABLE_CONVERSATION);
            List<ParseUser> part_list = pfconversation.getList(ConversationAdapter.PARTICIPANTS);
            part_list.add(currentUser);
        }
        chatmsg.put(ChatAdapter.CONVERSATION, pfconversation);
